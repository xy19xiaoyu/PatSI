...
namespace ExcelLib
{
    ...
    {
        public static DataTable Excel2DataTable(string filename, bool hasHeadRow, int SheetIndex = 1, int intfirstrow = 0)
        {
        ...
        public static DataTable Excel2DataTable(string filename, bool hasHeadRow, int SheetIndex = 1, int intfirstrow = 0, int top10 = 10)
        {
            using (FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read))
            {
                IWorkbook wb = WorkbookFactory.Create(fs);
                ...
                if (hasHeadRow)
                {
                    for (int i = 0; i < head.LastCellNum; i++)
                    {
                        if (!dt.Columns.Contains(head.Cells[i].ToString()))
                        {
                            dt.Columns.Add(new DataColumn(head.Cells[i].ToString()));
                        }
                    }
                ...
                for (int j = firstrow; j <= sheet.LastRowNum; j++)
                {
                    ...
                    for (int tmpj = head.FirstCellNum; tmpj < head.LastCellNum; tmpj++)
                    {
                        int tmpindex = tmpj - head.FirstCellNum;
                        if (dt.Columns.Count > tmpindex)
                        {
                            addrow[tmpindex] = row.GetCell(tmpj);
                        }
                    }
                    dt.Rows.Add(addrow);
                    if (dt.Rows.Count >= top10) break;
                }
                return dt;
            }
