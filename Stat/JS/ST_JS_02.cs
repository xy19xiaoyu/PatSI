...
{
    //技术生命周期
    public class ST_JS_02 : IStatAdapter
    {
        public ST_JS_02(string name, cfg config)
            : base(name, config)
        {
        ...
        public override string GetStatSQL()
        {
            ...
            columns = string.Format("{0}.{1} as '{2}'", Config.GroupYear.TableName, Config.GroupYear.ColName, Config.GroupYear.ShowName);
            ...
            orderby = string.Format("order by {0}.{1}", Config.GroupYear.TableName, Config.GroupYear.ColName);
