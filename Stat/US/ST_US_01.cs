...
namespace Stat.US
{
    public class ST_US_01 : IStatAdapter
    {


        public ST_US_01(string name, cfg config)
            : base(name, config)
        {

        }

        public ST_US_01(string name)
            : base(name)
        {
        ...
        public override string GetStatSQL()
        {
            ...
            where += string.Format(" and {0}.gj='美国' ", Config.GroupYear.TableName);
        ...
        public override bool ShowRightDataGridView(DataGridView rightgrid, ChartColumn col, string zid)
        {
            ...
            {
                sql = string.Format("select {0}.{1} as '{2}',count(distinct {0}.sid) as 专利数 from {0} where  ztid={3} and {0}.gj ='美国' group by {0}.{1} order by  {0}.{1} asc", col.TableName, col.ColName, col.ShowName, zid);
            }
            else
            {
                sql = string.Format("select {0}.{1} as '{2}',count(distinct {0}.sid) as 专利数 from {0},st_dt where {0}.sid=st_dt.sid and st_dt.ztid={3}  and {0}.gj ='美国' group by {0}.{1} order by {0}.{1} asc", col.TableName, col.ColName, col.ShowName, zid);
            }
