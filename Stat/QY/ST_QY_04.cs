...
namespace Stat.QY
{
    ...
    /// 区域发明人构成分析
    /// </summary>
    public class ST_QY_04 : IStatAdapter
    {
        public ST_QY_04(string name, cfg config)
            : base(name, config)
        {
        ...
        public override string GetStatSQL()
        {
            ...
            columns += string.Format(",{0}.{1} as '{2}'", Config.GroupIN.TableName, Config.GroupIN.ColName, Config.GroupIN.ShowName);
            groupby += string.Format(",{0}.{1}", Config.GroupIN.TableName, Config.GroupIN.ColName, Config.GroupIN.ShowName);
            tablenames.Add(Config.GroupIN.TableName);


            //orderby = string.Format("order by {0}.{1}", Config.GroupQY.TableName, Config.GroupQY.ColName);
            ...
            where += string.Format(" and {0}.{1} in({2})", Config.GroupIN.TableName, Config.GroupIN.ColName, Config.TopIN);
