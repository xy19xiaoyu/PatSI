...
namespace Stat.YJ
{
    public class ST_YJ_08 : IStatAdapter
    {


        public ST_YJ_08(string name, cfg config)
            : base(name, config)
        {

        }

        public ST_YJ_08(string name)
            : base(name)
        {
        ...
        {
            string sql1 = "select count(distinct st_dt.sid)   from st_dt,st_pns where st_dt.sid = st_pns.sid and  st_dt.ztid=" + Zid + " and st_pns.gj ='美国'";
            if (DBA.MySqlDbAccess.ExecuteScalar(CommandType.Text, "select dbtype from st_ztlist where id=" + Zid.ToString()).ToString() == "EPODOC")
            {
                sql1 = "select count(distinct st_dt.sid)   from st_dt where st_dt.ztid=" + Zid + " and st_dt.gj ='美国'";
            }
            ...
            else
            {
                ...
                row[4] = "US专利家族数量/总专利家族数量=" + d1.ToString("0.00");
            }
