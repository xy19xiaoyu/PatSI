...
{
    //技术生命周期
    public class ST_ZT_02 : IStatAdapter
    {
        public ST_ZT_02(string name, cfg config)
            : base(name, config)
        {
        ...
        public override string GetStatSQL()
        {
            ...
            columns = string.Format("{0}.{1} as '{2}'", Config.GroupPAType.TableName, Config.GroupPAType.ColName, Config.GroupPAType.ShowName);
            groupby = string.Format("{0}.{1}", Config.GroupPAType.TableName, Config.GroupPAType.ColName, Config.GroupPAType.ShowName);
            tablenames.Add(Config.GroupPAType.TableName);
            ...
            where += string.Format(" and {0}.{1} in({2})", Config.GroupPAType.TableName, Config.GroupPAType.ColName, Config.TopPAType);
