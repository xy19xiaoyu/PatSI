...
{
    //技术生命周期
    public class ST_ZT_05 : IStatAdapter
    {
        public ST_ZT_05(string name, cfg config)
            : base(name, config)
        {
        ...
        public override string GetStatSQL()
        {
            ...
            columns += string.Format(",{0}.{1} as '{2}'", Config.GroupType.TableName, Config.GroupType.ColName, Config.GroupType.ShowName);
            groupby += string.Format(",{0}.{1}", Config.GroupType.TableName, Config.GroupType.ColName, Config.GroupType.ShowName);

            tablenames.Add(Config.GroupType.TableName);
            ...
            where += string.Format(" and {0}.{1} in({2})", Config.GroupType.TableName, Config.GroupType.ColName, Config.TopType);
